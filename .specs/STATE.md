...
- **Main Interface** — primary application interface serving as central hub for image browsing activities; responsive layout with header (logo, global search bar, view toggle), collapsible sidebar (folder navigation tree, metadata filter panels), and main content area (breadcrumbs, image results in grid/list/timeline views, pagination); adapts to desktop/tablet/mobile viewports with sidebar overlay/drawer modes; handles empty states and maintains synchronized state across search, filter, and navigation components
