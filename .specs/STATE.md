...
- **Tags** (`/api/tags`) — tag CRUD, rename, merge, bulk delete, import from IPTC; per-image tag assignment (`/api/images/:id/tags`) with autocomplete/suggestion functionality, validation, and usage tracking; bulk tag operations across multiple images; searchable tag list with usage counts and filtering support for tag filter panel
