...
- **Image Detail** (`/image/:id`) — full metadata panel, GPS mini-map, tagging, keyboard navigation between images; accessible as modal overlay or deep link; displays full-resolution original images from source folders with organized metadata sections for camera info, file details, location, and IPTC data; includes zoom/pan support and RAW file preview handling; features context-aware return navigation with dynamic back button labels ("Back to search results", "Back to Hawaii photos") that preserve browsing context including search queries, active filters, scroll position, and selection state; supports URL parameter encoding for navigation context with graceful fallbacks for deep links
